...
model TapChangerBlocking5 "Tap Changer Blocking 5 (TCB5)"
  /* Tap Changer Blocking, special protection scheme, monitoring the voltage on five buses going below their predefined threshold
  ...
  parameter Types.VoltageModule UMin5 "Minimum voltage 5 threshold before tap-changer blocking";
  ...
  Dynawo.Connectors.ImPin UMonitored5 "Monitored voltage 5";

equation
  // Check when the monitored voltage goes below UMin
  when UMonitored1.value < UMin1 or UMonitored2.value < UMin2 or UMonitored3.value < UMin3 or UMonitored4.value < UMin4 or UMonitored5.value < UMin5 then
    UUnderMin = true;
    tUnderUmin = time;
  elsewhen UMonitored1.value >= UMin1 and UMonitored2.value >= UMin2 and UMonitored3.value >= UMin3 and UMonitored4.value >= UMin4 and UMonitored5.value >= UMin5 and pre(UUnderMin) then
    UUnderMin = false;
    tUnderUmin = Constants.inf;
  end when;

  annotation(preferredView = "text",
    Documentation(info = "<html><head></head><body>this model will send a block order to transformers with tap-changers to block the tap to its current step if the voltage becomes lower than a threshold on one of five controlled nodes. Such a mechanism enables to avoid voltage collapse on situations where the tap actions become negative for the system stability.<div><br></div>The detailed tap-changer-blocking behavior is explained in the following state diagram:

<figure>
    <img width=\"450\" src=\"modelica://Dynawo/Electrical/Controls/Voltage/Images/TapChangerBlock.png\">
</figure>
</body></html>"));
end TapChangerBlocking5;
