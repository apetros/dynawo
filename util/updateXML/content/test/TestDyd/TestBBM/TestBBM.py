...
class TestBBM(unittest.TestCase):
    ...
    def test_change_library_name(self):
        test_dir_path = os.path.join(parent_dir, "res/change_library_name")
        update_XML_script = os.path.join(test_dir_path, "change_library_name.py")
        launch_test(self, test_dir_path, update_XML_script, test_dir_path)

    def test_change_connect_var_name(self):
        test_dir_path = os.path.join(parent_dir, "res/change_connect_var_name")
    ...
    def test_remove_macro_connect(self):
        test_dir_path = os.path.join(parent_dir, "res/remove_macro_connect")
        update_XML_script = os.path.join(test_dir_path, "remove_macro_connect.py")
        launch_test(self, test_dir_path, update_XML_script, test_dir_path)
    ...
    def test_get_connects(self):
        ...
        gen = dyds.get_bbms(lambda bbm: bbm.get_id() == "GEN____1_SM")
        gen_connects = gen[0].connects.get_connects()
