<!--  This xsl:
    * rename tapChangerLock_lockedD in tapChangerBlocking_blockedD
-->
    ...
    <xsl:template match="@var1">
        ...
        <xsl:attribute name="var1">
            <xsl:call-template name="string-replace-all">
                ...
                <xsl:with-param name="replace" select="'tapChangerLock_lockedD'" />
                <xsl:with-param name="by" select="'tapChangerBlocking_blockedD'" />
            </xsl:call-template>
        </xsl:attribute>
    </xsl:template>
