...
<xsl:template match="dyn:par">
  <xsl:choose>
    <xsl:when test="@name='ExcitationPu' and not(../dyn:par[@name='nd'])">
      <xsl:copy-of select="."/>
      ...
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">DOUBLE</xsl:attribute>
        <xsl:attribute name="name">mq</xsl:attribute>
        <xsl:attribute name="value">0</xsl:attribute>
      </xsl:element>
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">DOUBLE</xsl:attribute>
      ...
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">DOUBLE</xsl:attribute>
        <xsl:attribute name="name">nq</xsl:attribute>
        <xsl:attribute name="value">0</xsl:attribute>
      </xsl:element>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template match="par">
  <xsl:choose>
    <xsl:when test="@name='ExcitationPu' and not(../par[@name='nd'])">
      <xsl:copy-of select="."/>
