...
<xsl:template match="dyn:par">
  <xsl:choose>
    <xsl:when test="@name='hvdc_KLosses' and not(../dyn:par[@name='hvdc_PMaxPu'])">
      <xsl:copy-of select="."/>
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">DOUBLE</xsl:attribute>
        <xsl:attribute name="name">hvdc_PMaxPu</xsl:attribute>
...
<xsl:template match="par">
  <xsl:choose>
    <xsl:when test="@name='hvdc_KLosses' and not(../par[@name='hvdc_PMaxPu'])">
      <xsl:copy-of select="."/>
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">DOUBLE</xsl:attribute>
        <xsl:attribute name="name">hvdc_PMaxPu</xsl:attribute>
