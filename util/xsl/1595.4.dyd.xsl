<!--  This xsl:
    * renames event_state2 by event_state2_value for load variation events on load_QRefPu
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dyn="http://www.rte-france.com/dynawo">
    ...
    <xsl:template match="@var2">
        <xsl:choose>
            <xsl:when test="../@var1='load_QRefPu'">
                <xsl:variable name="name" select="."/>
                <xsl:attribute name="var2">
                    <xsl:call-template name="string-replace-all">
                        <xsl:with-param name="text" select="$name" />
                        <xsl:with-param name="replace" select="'event_state2'" />
                        <xsl:with-param name="by" select="'event_state2_value'" />
                    </xsl:call-template>
                </xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="@var1">
        <xsl:choose>
            <xsl:when test="../@var2='load_QRefPu'">
                <xsl:variable name="name" select="."/>
