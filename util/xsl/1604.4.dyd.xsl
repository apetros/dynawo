<!--  This xsl:
    * rename TapChangerBlockingArea in TapChangerBlockingAutomaton1
-->
    ...
    <xsl:template match="@lib">
        ...
        <xsl:attribute name="lib">
            <xsl:call-template name="string-replace-all">
                ...
                <xsl:with-param name="replace" select="'TapChangerBlockingArea'" />
                <xsl:with-param name="by" select="'TapChangerBlockingAutomaton1'" />
            </xsl:call-template>
        </xsl:attribute>
    </xsl:template>
</xsl:stylesheet>
