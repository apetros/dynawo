...
<xsl:template match="dyn:par">
  <xsl:choose>
    <xsl:when test="@name='generator_ExcitationPu' and not(../dyn:par[@name='generator_UseApproximation'])">
      <xsl:copy-of select="."/>
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">BOOL</xsl:attribute>
        <xsl:attribute name="name">generator_UseApproximation</xsl:attribute>
...
<xsl:template match="par">
  <xsl:choose>
    <xsl:when test="@name='generator_ExcitationPu' and not(../par[@name='generator_UseApproximation'])">
      <xsl:copy-of select="."/>
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">BOOL</xsl:attribute>
        <xsl:attribute name="name">generator_UseApproximation</xsl:attribute>
